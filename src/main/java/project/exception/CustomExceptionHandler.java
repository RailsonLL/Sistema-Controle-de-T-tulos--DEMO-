		...
		while (iterator.hasNext()) {
			...
			try {
				...
				if (exception != null && exception.getMessage() != null && exception.getMessage().indexOf("ConstraintViolationException") != -1) {
					...
					FacesContext.getCurrentInstance().addMessage("msg", new FacesMessage(FacesMessage.SEVERITY_FATAL, "O sitema se recuperou de um erro inesperado.", ""));
					...
					//PrimeFaces - esse alerta sera exibido se a página não redirecionar
					org.primefaces.context.RequestContext.getCurrentInstance().execute("alert('O sistema se recuperou de um erro inesperado.')");
				
					//PrimeFaces - mostra a mensagem em uma caixa de dialogo
